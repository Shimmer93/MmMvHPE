...
TRAIN_CFG="${TRAIN_CFG:-${REPO_ROOT}/configs/baseline/occlusion_robustness/panoptic_seq3_panoptic_train_temporal_fg.yml}"
...
EXP_NAME="${EXP_NAME:-occlusion_robustness_seq3}"
...
echo "[seq3-occlusion] training config: ${TRAIN_CFG}"
...
if [[ -z "${BEST_CKPT}" || ! -f "${BEST_CKPT}" ]]; then
  echo "[seq3-occlusion] ERROR: no checkpoint found under ${CKPT_DIR}" >&2
  exit 1
fi

echo "[seq3-occlusion] testing checkpoint: ${BEST_CKPT}"
