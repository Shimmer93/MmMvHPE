...
The system SHALL use the existing Mamba4D point‑cloud encoder to produce per‑frame features
and regress a sequence of 3D keypoints shaped `B, T, J, 3` for HuMMan.
...
The training pipeline SHALL compute uniform per‑frame MSE loss between predicted and GT
3D keypoint sequences.
...
The system SHALL provide a HuMMan config that trains the Mamba4D sequence pose estimator
using point‑cloud inputs only.
