...
Define the config-driven workflow for evaluating SAM-3D-Body on preprocessed Panoptic data at the segment level, including single-camera selection from multiview configs, Panoptic COCO19 joint adaptation, per-segment metric reporting, and worst-case visualization export.
