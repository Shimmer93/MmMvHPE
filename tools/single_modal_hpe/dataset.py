...
class PanopticDepthToLidarDataset(PanopticPreprocessedDatasetV1):
    """Depth-only Panoptic dataset that uses the v1 depth->LiDAR conversion path."""
    ...
    def _rotate_xyz_data(data, rotation: np.ndarray):
        ...
        if isinstance(data, np.ndarray):
            ...
            return out
        if isinstance(data, list):
            return [PanopticDepthToLidarDataset._rotate_xyz_data(x, rotation) for x in data]
        if isinstance(data, tuple):
            return [PanopticDepthToLidarDataset._rotate_xyz_data(x, rotation) for x in data]
        return data
