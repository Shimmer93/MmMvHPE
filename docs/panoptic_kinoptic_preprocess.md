...
Optional foreground-only depth generated by:
- `scripts/build_depth_background_map.sh`
- `scripts/extract_subject_depth_from_background.sh`

When generated, per-sequence layout is:
- `<out_root>/<sequence>/depth_fg/kinect_<node>/<body_frame_id>.png`

To use this in `PanopticPreprocessedDatasetV1`, set:
- `use_foreground_depth: true`
- `foreground_depth_subdir: depth_fg`

Optional GT/camera anchoring (HuMMan-style):
- `anchor_key: input_rgb` anchors GT keypoints and camera extrinsics to RGB camera coordinates.
- Supported values depend on enabled modalities: `input_rgb`, `input_depth`, `input_lidar`.
- If unset, loader keeps the existing coordinate convention.
...
Example: Panoptic model-comparison configs (foreground depth + temporal split):

```bash
# Train PanopticHPE (seq3) on non-piano sequences.
uv run python main.py \
  -c configs/baseline/occlusion_robustness/panoptic_seq3_panoptic_train_temporal_fg.yml

# Train XFi on the same Panoptic regime with RGB-anchored GT.
uv run python main.py \
  -c configs/baseline/occlusion_robustness/xfi_panoptic_train_temporal_fg_rgb_anchor.yml

# Test on occluded subset (office1/office2/cello3).
uv run python main.py \
  -c configs/baseline/occlusion_robustness/panoptic_seq3_panoptic_test_occluded.yml \
  --test
```
