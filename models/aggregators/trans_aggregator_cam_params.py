...
class TransformerAggregatorWithCameraParams(nn.Module):
    """
    Transformer aggregator that injects camera intrinsics/extrinsics as tokens.
    Camera parameters are encoded with absT + quaternion + FoV and added to camera tokens.
    """
    ...
    def __init__(
        ...
        require_camera_params: bool = True,
        normalize_camera_params: bool = True,
        camera_param_gate_init: float = -4.0,
    ):
        super().__init__()
        ...
        self.camera_param_proj = nn.Linear(camera_param_dim, embed_dim, bias=proj_bias)
        self.camera_param_norm = nn.LayerNorm(embed_dim)
        self.pose_encoding_type = pose_encoding_type
        self.use_camera_params = use_camera_params
        self.require_camera_params = require_camera_params
        self.normalize_camera_params = normalize_camera_params
        self.camera_param_gate = nn.Parameter(torch.tensor(float(camera_param_gate_init)))
    ...
    def forward(self, features, **kwargs):
        ...
        if B == 0:
            raise ValueError("TransformerAggregatorWithCameraParams requires at least one non-empty modality.")
        ...
        active_modalities = [
            features_rgb is not None,
            features_depth is not None,
            features_lidar is not None,
            features_mmwave is not None,
        ]
        camera_param_tokens = self._build_camera_param_tokens(
            kwargs,
            B,
            T,
            device=camera_tokens_normal.device,
            dtype=camera_tokens_normal.dtype,
        ...
        for i, feat in enumerate([features_rgb, features_depth, features_lidar, features_mmwave]):
            if feat is not None:
                if i == anchor_idx:
                    base_camera_tokens = camera_tokens_anchor
                    joint_tokens = joint_tokens_anchor
                    register_tokens = register_tokens_anchor
                else:
                    base_camera_tokens = camera_tokens_normal
                    joint_tokens = joint_tokens_normal
                    register_tokens = register_tokens_normal

                camera_tokens = base_camera_tokens
                if camera_param_tokens[i] is not None:
                    camera_tokens = camera_tokens + camera_param_tokens[i]

                feat = self.insert_special_tokens(feat, camera_tokens, joint_tokens, register_tokens)
    ...
    def _build_camera_param_tokens(self, kwargs, B, T, device, dtype, active_modalities):
        tokens = [None, None, None, None]
        if not self.use_camera_params:
            return tokens

        for idx, modality in enumerate(self.modalities):
            pose_enc = self._get_camera_pose_encoding(kwargs, modality, B, T, device, dtype)
            if pose_enc is None:
                if self.require_camera_params and active_modalities[idx]:
                    raise KeyError(
                        f"Missing camera parameters for modality '{modality}'. "
                        f"Expected '{'gt_camera_' + modality}' or '{modality}_camera' + 'input_{modality}'."
                    )
                continue
            if pose_enc.dim() == 2:
                if pose_enc.shape[0] == B:
                    pose_enc = pose_enc.unsqueeze(1)
                elif pose_enc.shape[0] == T:
                    pose_enc = pose_enc.unsqueeze(0)
                else:
                    continue
            if pose_enc.shape[0] != B and pose_enc.shape[0] == 1:
                pose_enc = pose_enc.expand(B, -1, -1)
            if pose_enc.shape[1] != T and pose_enc.shape[1] == 1:
                pose_enc = pose_enc.expand(-1, T, -1)
            pose_enc = pose_enc.to(device=device, dtype=dtype)
            if self.normalize_camera_params:
                pose_enc = self._normalize_pose_encoding(pose_enc)
            token = self.camera_param_proj(pose_enc)
            token = self.camera_param_norm(token).unsqueeze(2)
            gate = torch.sigmoid(self.camera_param_gate)
            tokens[idx] = token * gate
        return tokens

    def _get_camera_pose_encoding(self, kwargs, modality, B, T, device, dtype):
        gt_key = f"gt_camera_{modality}"
        if gt_key in kwargs and kwargs[gt_key] is not None:
            return self._as_tensor(kwargs[gt_key], device, dtype)
    ...
    def _stack_camera_matrices(self, camera, B, device, dtype):
        ...
        if extrinsics.dim() != 3 or extrinsics.shape[-2:] != (3, 4):
            return None, None
    ...
    def _normalize_pose_encoding(self, pose_enc):
        mean = pose_enc.mean(dim=(0, 1), keepdim=True)
        std = pose_enc.std(dim=(0, 1), keepdim=True).clamp_min(1e-6)
        return (pose_enc - mean) / std
