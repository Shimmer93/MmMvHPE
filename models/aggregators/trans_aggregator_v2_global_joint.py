...
class TransformerAggregatorV2GlobalJoint(nn.Module):
    def __init__(
        self,
                ...
                match aa_type:
                    case "single":
                        modality_tokens, single_idx, intermediates = self._process_single_attention(
                            modality_tokens, joint_tokens, Ns, single_idx, pos=None
                        )
                    case "cross_joint":
                        joint_tokens, cross_idx, intermediates = self._process_cross_joint_attention(
                            joint_tokens, modality_tokens, Ns, cross_idx, pos=None
                        )
                    case "gcn":
                        joint_tokens, gcn_idx, intermediates = self._process_gcn(
                            joint_tokens, modality_tokens, Ns, gcn_idx
                        )
                    case _:
    ...
    def _process_single_attention(self, modality_tokens, joint_tokens, Ns, idx, pos=None):
        intermediates = []
        ...
        for _ in range(self.aa_block_size):
            ...
            tokens_base = updated
            idx += 1
            intermediates.append(self._extract_output_tokens(tokens_base, joint_tokens, Ns))

        return tokens_base, idx, intermediates

    def _process_cross_joint_attention(self, joint_tokens, modality_tokens, Ns, idx, pos=None):
        intermediates = []
        ...
        for _ in range(self.aa_block_size):
            for tokens in modality_tokens:
                if tokens is None:
                    continue
                B, T, N, C = tokens.shape
                queries = joint_base.reshape(B, T * self.num_joints, C)
                ...
                joint_base = queries.reshape(B, T, self.num_joints, C)

            idx += 1
            intermediates.append(self._extract_output_tokens(modality_tokens, joint_base, Ns))

        return joint_base, idx, intermediates

    def _process_gcn(self, joint_tokens, modality_tokens, Ns, idx):
        intermediates = []
        joint_base = joint_tokens
    ...
    def _extract_output_tokens(self, modality_tokens, joint_tokens, Ns):
        ...
        for tokens in modality_tokens:
            ...
            token_slices.append(torch.cat([camera_tokens, joint_tokens], dim=2))
